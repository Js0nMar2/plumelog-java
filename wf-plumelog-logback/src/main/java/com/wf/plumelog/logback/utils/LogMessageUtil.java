package com.wf.plumelog.logback.utils;
...
import com.wf.plumelog.core.util.DateUtil;
import com.wf.plumelog.core.util.LogExceptionStackTrace;
import com.wf.plumelog.core.util.StringUtils;
import com.wf.plumelog.core.util.TraceLogMessageFactory;
...
public class LogMessageUtil {
    ...
    public static String getLogMessage(BaseLogMessage baseLogMessage, final ILoggingEvent iLoggingEvent) {
        ...
        if (mdc != null && mdc.size() > 0) {
            mdc.forEach((k, v) -> {
                if ("costTime".equals(k)) {
                    map.put("costTime", Integer.parseInt(mdc.get("costTime")));
                    return;
                }
                map.put(k, v);
            });
